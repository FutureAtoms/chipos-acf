...
async def database_metrics():
    ...
    try:
        ...
        # Get projects count
        projects_response = (
            supabase_client.table("chipos_projects").select("id", count="exact").execute()
        )
        ...
        # Get tasks count
        tasks_response = supabase_client.table("chipos_tasks").select("id", count="exact").execute()
        tables_info["tasks"] = tasks_response.count if tasks_response.count is not None else 0

        # Get crawled pages count
        pages_response = (
            supabase_client.table("chipos_crawled_pages").select("id", count="exact").execute()
        )
        ...
        # Get settings count
        settings_response = (
            supabase_client.table("chipos_settings").select("id", count="exact").execute()
        )
