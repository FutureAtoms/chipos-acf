...
These endpoints are meant to be called only by other services in the ChipOS system,
...
ALLOWED_INTERNAL_IPS = [
    ...
    "chipos-agents",  # Docker service name
    "chipos-mcp",  # Docker service name
]
...
async def get_agent_credentials(request: Request) -> dict[str, Any]:
    ...
    try:
        ...
        credentials = {
            ...
            # MCP endpoint
            "MCP_SERVICE_URL": f"http://chipos-mcp:{os.getenv('ARCHON_MCP_PORT')}",
            # Additional settings
