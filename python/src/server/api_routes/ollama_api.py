...
class StoredModelInfo(BaseModel):
    """Stored model information with ChipOS compatibility assessment."""
    name: str
    host: str
    ...
    chipos_compatibility: str  # 'full', 'partial', 'limited'
...
async def discover_and_store_models_endpoint(request: ModelDiscoveryAndStoreRequest) -> ModelListResponse:
    """
    Discover models from Ollama instances, assess ChipOS compatibility, and store in database.
    
    This endpoint fetches detailed model information from configured Ollama instances,
    evaluates their compatibility with ChipOS features, and stores the results for
    ...
    try:
        ...
        for instance_url in request.instance_urls:
            try:
                ...
                for model in models:
                    # Assess ChipOS compatibility
                    compatibility_info = _assess_chipos_compatibility(model)
                    ...
                    stored_model = StoredModelInfo(
                        ...
                        chipos_compatibility=compatibility_info['level'],
        ...
        # Store models in chipos_settings
        models_data = {
        ...
        # Upsert into chipos_settings table
        result = supabase.table("chipos_settings").upsert({
...
async def get_stored_models_endpoint() -> ModelListResponse:
    ...
    try:
        ...
        # Get stored models from chipos_settings
        result = supabase.table("chipos_settings").select("value").eq("key", "ollama_discovered_models").execute()
            ...
            try:
                ...
                if isinstance(model, dict):
                    stored_model = StoredModelInfo(
                        ...
                        chipos_compatibility=model.get('chipos_compatibility', 'unknown'),
...
async def _assess_chipos_compatibility_with_testing(model, instance_url: str) -> dict[str, Any]:
    """Assess ChipOS compatibility for a given model using actual capability testing."""
...
def _assess_chipos_compatibility(model) -> dict[str, Any]:
    """Legacy compatibility assessment for backward compatibility. Consider using _assess_chipos_compatibility_with_testing for new code."""
    model_name = model.name.lower()
    capabilities = getattr(model, 'capabilities', [])
...
async def discover_models_with_real_details(request: ModelDiscoveryAndStoreRequest) -> ModelDiscoveryResponse:
    ...
    try:
                    ...
                    for model_data in tags_data["models"]:
                        ...
                        try:
                            ...
                            stored_model = StoredModelInfo(
                                ...
                                chipos_compatibility=compatibility['level'],
        ...
        result = supabase.table("chipos_settings").update({
            "value": json.dumps(models_data),
