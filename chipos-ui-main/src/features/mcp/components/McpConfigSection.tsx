...
export const McpConfigSection: React.FC<McpConfigSectionProps> = ({ config, status, className }) => {
  ...
  const handleCursorOneClick = () => {
    ...
    const deeplink = `cursor://anysphere.cursor-deeplink/mcp/install?name=chipos&config=${base64Config}`;
    window.location.href = deeplink;
    showToast("Opening Cursor with ChipOS MCP configuration...", "info");
  };

  const handleClaudeCodeCommand = () => {
    const command = `claude mcp add --transport http chipos http://${config.host}:${config.port}/mcp`;
    ...
    <div className={cn("space-y-6", className)}>
      ...
      <div className={cn("p-3 rounded-lg", glassmorphism.background.blue, glassmorphism.border.blue)}>
        <p className="text-sm text-blue-700 dark:text-blue-300">
          <span className="font-semibold">Note:</span> ChipOS works with any application that supports MCP. Below are
          ...
          {selectedIDE === "claudecode" && (
            ...
            >
              <code className="text-sm font-mono text-cyan-600 dark:text-cyan-400">
                claude mcp add --transport http chipos http://{config.host}:{config.port}/mcp
              </code>
