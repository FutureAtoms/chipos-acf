...
```tsx
import { ChipOSChatPanel } from '@/components/agent-chat/ChipOSChatPanel';
...
async def process_agent_response(session_id: str, message: str, context: dict):
    ...
    async with httpx.AsyncClient() as client:
        async with client.stream(
            "POST",
            f"http://chipos-agents:8052/agents/rag/stream",
            json={
                "prompt": message,
                "context": context
            }
...
- **Context**: `{ match_count: 5, source_filter: "docs.chipos.com" }`
...
AGENT_SERVICE_URL=http://chipos-agents:8052
...
- Verify server logs: `docker logs chipos-server`
