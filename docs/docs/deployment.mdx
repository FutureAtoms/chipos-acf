---
title: Deployment
description: Quick start guide for running ChipOS
sidebar_position: 10
---
...
ChipOS automatically includes Hot Module Reload for both Python server and React UI. Code changes are reflected immediately without container rebuilds.
</Admonition>
...
```bash
docker-compose restart chipos-mcp
```
</Admonition>
...
graph LR
    ...
    subgraph "Docker Containers"
        Frontend[chipos-frontend<br/>React UI]
        API[ChipOS-Server<br/>FastAPI + Socket.IO]
        MCP[chipos-mcp<br/>MCP Server]
        Agents[chipos-agents<br/>AI Processing]
        Docs[chipos-docs<br/>Documentation]
    end
...
# Restart a service
docker-compose restart chipos-frontend
```
...
# View service logs
docker logs chipos-server
docker logs chipos-mcp
docker logs chipos-agents
...
<Admonition type="info" title="Scaling ChipOS">
...
Your ChipOS instance is now running. Next:
