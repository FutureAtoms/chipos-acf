...
The MCP service provides a Model Context Protocol interface for AI clients to access ChipOS's functionality via HTTP.
...
ChipOS MCP server uses **SSE (Server-Sent Events) transport only**.
