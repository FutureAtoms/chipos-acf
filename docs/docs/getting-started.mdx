...
By the end of this guide, you'll have a **fully operational ChipOS system** with:
- ✅ **14 MCP tools** enabled and working
- 🧠 **RAG system** for intelligent knowledge retrieval
- 🌐 **ChipOS UI** Command Center for knowledge, projects, and tasks
...
ChipOS is a **Model Context Protocol (MCP) server** that creates a centralized knowledge base for your AI coding assistants. Connect Cursor, Windsurf, or Claude Desktop to give your AI agents access to:
...
```bash
git clone https://github.com/coleam00/chipos.git
cd chipos
...
ChipOS includes Hot Module Reload (HMR) for both Python and React code. Changes to source files automatically reload without container rebuilds.
...
Your ChipOS system is now running with **14 MCP tools** available. Your AI agents can now access your knowledge base!
</Admonition>
...
graph TB
    subgraph "🚀 Docker Microservices Stack"
        Frontend["🌐 Frontend Container<br/>React + Vite<br/>chipos-frontend<br/>Port 3737"]
        API["⚡ Server Container<br/>FastAPI + Socket.IO<br/>ChipOS-Server<br/>Port 8080"]
        MCP["🔧 MCP Server<br/>14 MCP Tools<br/>chipos-mcp<br/>Port 8051"]
        Agents["🤖 Agents Service<br/>AI Processing<br/>chipos-agents<br/>Port 8052"]
        Docs["📚 Documentation<br/>Docusaurus<br/>chipos-docs<br/>Port 3838"]
    end
...
To connect Cursor to your ChipOS MCP server, add this configuration:
...
**QuickStart guide** to get ChipOS running in minutes
...
**[Deployment Guide](./deployment.mdx)** - Scale ChipOS for team and production use
...
docker compose logs chipos-server
docker compose logs chipos-mcp
docker compose logs chipos-agents
```
...
docker compose exec chipos-server env | grep -E "(SUPABASE|OPENAI)"
...
- **🐛 [GitHub Issues](https://github.com/coleam00/chipos/issues)** - Bug reports and feature requests
...
<p align="center">
  ...
  <em>Transform your AI coding experience with ChipOS</em>
</p> 