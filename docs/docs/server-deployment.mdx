...
<div className="hero hero--primary">
  <div className="container">
    <h2 className="hero__subtitle">
      Deploy ChipOS with Docker: **Production-ready microservices** with health checks, scaling, and monitoring
    </h2>
  </div>
</div>

This guide covers deploying ChipOS's server architecture using Docker and Docker Compose, including configuration, scaling, and production best practices.
...
├── chipos-ui-main/              # React frontend application
...
services:
  ...
  # Frontend
  frontend:
    build: ./chipos-ui-main
    container_name: chipos-frontend
    ports:
      - "3737:5173"
    environment:
    ...
    healthcheck:
      ...
      retries: 3
    volumes:
      - ./chipos-ui-main/src:/app/src
      - ./chipos-ui-main/public:/app/public
    depends_on:
      - chipos-server
...
1. **Clone the repository**
   ```bash
   git clone https://github.com/chipos/chipos.git
   cd chipos
   ```
...
docker compose up -d --scale chipos-agents=3

# Scale the Server service for more concurrent requests
docker compose up -d --scale chipos-server=2
...
docker inspect chipos-server | grep -A 5 Health
...
docker exec chipos-server python -c "
...
# Push to registry
docker tag chipos-server:latest your-registry/chipos-server:v1.0.0
docker push your-registry/chipos-server:v1.0.0
```
...
# Deploy stack
docker stack deploy -c docker-compose.yml chipos
```
