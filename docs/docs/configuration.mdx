---
title: Configuration
description: Essential configuration for ChipOS
sidebar_position: 3
---
...
ChipOS uses a microservices architecture with three separate services. All services run in Docker containers and communicate via the `app-network` bridge network using service names, not localhost. Real-time features use Socket.IO for reliable real-time communication.
</Admonition>

```mermaid
graph TB
    subgraph "Docker Network: app-network"
        subgraph "ChipOS-Server"
            API[FastAPI<br/>:8080]
...
|---------|----------------|----------------|-----------|---------|
| Frontend | chipos-frontend | 5173 | 3737 | React UI |
| Server | ChipOS-Server | 8080 | 8080 | FastAPI + Socket.IO |
| MCP Server | chipos-mcp | 8051 | 8051 | AI Agent connections |
| AI Agents | chipos-agents | 8052 | 8052 | AI processing service |
| Documentation | chipos-docs | 80 | 3838 | This documentation |
...
ChipOS includes a unified logging system that works seamlessly with or without Logfire:
...
ChipOS supports multiple LLM providers through the Settings UI:
...
ChipOS uses Socket.IO for all real-time features. The default configuration works out of the box:
...
- Frontend → Server: `http://ChipOS-Server:8080`
- API → MCP: `http://chipos-mcp:8051`
- API → Agents: `http://chipos-agents:8052`
...
proxy: {
  '/api': {
    target: 'http://ChipOS-Server:8080',
    ws: true
  }
}
```
...
- **Location**: `chipos-ui-main/src/services/serverHealthService.ts`
...
docker compose build chipos-server
docker compose up -d chipos-server

# View real-time logs for a service
docker compose logs -f chipos-server
...
ChipOS includes automatic service discovery that works across environments:
...
# Docker: http://ChipOS-Server:8080
