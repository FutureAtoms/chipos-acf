...
The Server service is the core of ChipOS, containing all business logic, services, and data operations.
...
```yaml
services:
  chipos-server:     # FastAPI + Socket.IO (port 8080)
  chipos-mcp:        # MCP Server (port 8051)
  chipos-agents:     # AI Agents (port 8052)
  chipos-frontend:   # React UI (port 3737)
  chipos-docs:       # Documentation (port 3838)
```
...
AGENTS_BASE_URL=http://chipos-agents:8052
```
...
1. **Clone repository**
   ```bash
   git clone https://github.com/ArchonInnovations/chipos.git
   cd chipos
   ```
