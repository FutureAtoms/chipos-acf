...
**ChipOS's MCP implementation follows a clear separation of concerns:**
...
The Model Context Protocol (MCP) is an open standard that enables AI applications to securely access external data sources and tools. ChipOS implements both sides of MCP:
...
<div className="row">
  <div className="col col--6">
    <div className="card">
      ...
      <div className="card__body">
        <p>ChipOS as an MCP Server - Expose your knowledge base to AI clients</p>
        <ul>
  ...
  <div className="col col--6">
    <div className="card">
      ...
      <div className="card__body">
        <p>Universal MCP Client - Connect to any SSE-based MCP server from ChipOS's UI</p>
        <ul>
...
graph TB
    ...
    %% ChipOS's Client can also connect to its own server
...
<Tabs>
<TabItem value="server" label="Using ChipOS as MCP Server" default>

Connect your AI client to ChipOS's knowledge base:

1. Start ChipOS (Docker or local)
...
**For Claude Code:**
```bash
claude mcp add --transport sse chipos http://localhost:8051/sse
```
...
Connect to any MCP server from ChipOS's UI:

1. Open ChipOS UI → Navigate to MCP Client tab
...
- **ChipOS**: Your own knowledge base
...
**ChipOS provides 14 comprehensive MCP tools following modern patterns:**
...
## 🔥 Why Choose ChipOS's MCP Implementation?
...
**ChipOS's MCP implementation prioritizes developer experience with comprehensive tooling, detailed documentation, and real-time debugging capabilities.**
...
|----------|---------|----------|
| **[🛠️ MCP Server Guide](/mcp-server)** | Complete server setup, configuration, and troubleshooting | Developers using ChipOS as an MCP server |
...
Choose your path based on how you want to use ChipOS's MCP capabilities:

<Tabs>
<TabItem value="expose" label="🛠️ Expose ChipOS as MCP Server">

**You want to connect external AI clients (Cursor, Windsurf, etc.) to ChipOS's knowledge base**
...
**You want to use ChipOS's UI to connect to and test various MCP servers**
...
**You want to use ChipOS as both a server and client for maximum functionality**
...
<div className="text--center">
  ...
  <p>Join thousands of developers already using ChipOS's MCP tools to supercharge their AI workflows!</p>
</div> 